-- Fix job notifications to notify drivers when jobs are created
...
-- Step 2: Create new comprehensive notification function
...
DECLARE
    ...
    online_drivers_count INTEGER := 0;
BEGIN
    -- Handle INSERT (new job created)
    IF TG_OP = 'INSERT' THEN
        RAISE NOTICE 'New job created: % - %', NEW.id, NEW.title;
        
        -- Get all online drivers and create notifications for them
        ...
        LOOP
            ...
            online_drivers_count := online_drivers_count + 1;
        END LOOP;
        
        RAISE NOTICE 'Created notifications for % online drivers', online_drivers_count;
...
-- Step 5: Test the new notification system
DO $$
DECLARE
    test_job_id UUID;
...
BEGIN
    RAISE NOTICE '=== TESTING NEW NOTIFICATION SYSTEM ===';
    
    -- Count notifications before
    SELECT COUNT(*) INTO notification_count_before FROM notifications;
    
    -- Count online drivers
    SELECT COUNT(*) INTO online_drivers_count FROM users WHERE role = 'driver' AND online_status = 'online';
    
    RAISE NOTICE 'Notifications before: %', notification_count_before;
    RAISE NOTICE 'Online drivers: %', online_drivers_count;
    
    -- Create a test job (this should trigger notifications for all online drivers)
    INSERT INTO jobs (
        title, description, location, rate, status, admin_id, created_at, updated_at
    ) VALUES (
        'Test Job for Drivers',
    ...
    ELSE
        ...
        RAISE NOTICE 'This might be because there are no online drivers';
    END IF;
