-- Check Online Column Constraints and Dependencies
...
-- Step 4: Check for functions that might reference these columns
SELECT 
    'Functions Referencing Online Columns' as info,
    p.proname as function_name,
    pg_get_functiondef(p.oid) as function_definition
FROM pg_proc p
JOIN pg_namespace n ON p.pronamespace = n.oid
WHERE n.nspname = 'public'
    AND (pg_get_functiondef(p.oid) LIKE '%users.online%' 
         OR pg_get_functiondef(p.oid) LIKE '%users.is_online%' 
         OR pg_get_functiondef(p.oid) LIKE '%users.online_status%');

-- Step 5: Check for RLS policies that reference these columns
SELECT 
...
-- Step 6: Check for indexes on these columns
SELECT 
...
-- Step 7: Check for any other tables that might reference these columns
SELECT 
...
-- Step 8: Check for any application code references (if we can find them)
SELECT 
