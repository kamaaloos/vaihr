...
-- Verify the view exists and has correct security settings
DO $$ 
BEGIN
    IF EXISTS (
        SELECT 1 
        ...
        -- Verify security_invoker setting using pg_class
        IF EXISTS (
            SELECT 1
            FROM pg_class c
            JOIN pg_namespace n ON n.oid = c.relnamespace
            WHERE n.nspname = 'public'
            AND c.relname = 'jobs_with_admin'
            AND c.relkind = 'v'
            AND NOT c.relsecurity_invoker = false
        ) THEN
            RAISE NOTICE 'View jobs_with_admin has security_invoker = true';
        ELSE
            RAISE NOTICE 'WARNING: View jobs_with_admin does not have security_invoker = true';
        END IF;
    ELSE
