...
async function deleteExistingUser() {
    try {
        ...
        if (dbUser) {
            ...
            console.log('Updating jobs to remove driver references...');
        ...
        const driverUser = users.users.find(u => u.email === 'kamaaloos@gmail.com');

        if (driverUser) {
            console.log('Found existing user in auth system, deleting...');
            const { error: deleteAuthError } = await supabaseMigration.auth.admin.deleteUser(driverUser.id);
            if (deleteAuthError) {
...
async function createDriver() {
    try {
        console.log('Starting driver user creation process...');

        // First, clean up any existing driver user
        console.log('Cleaning up existing driver user...');
        ...
        console.log('Creating new driver user in auth system...');
        ...
        console.log('Email: kamaaloos@gmail.com');
