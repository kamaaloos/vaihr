...
BEGIN
    ...
    SELECT 
        ...
        jobs.rate,
        jobs.status,
        CASE WHEN jobs.driver_id IS NULL THEN NULL ELSE jobs.driver_id::uuid END,
        CASE WHEN jobs.admin_id IS NULL THEN NULL ELSE jobs.admin_id::uuid END,
        jobs.driver_name,
        jobs.image_url,
        jobs.created_at,
        jobs.updated_at,
